...
function configure(options) {
    const { enforceActions, computedRequiresReaction, disableErrorBoundaries, arrayBuffer, reactionScheduler } = options;
...
class ObservableObjectAdministration {
    ...
    getKeys() {
        if (this.keys === undefined) {
            this.keys = (new ObservableArray(getPlainObjectKeys(this.values).filter(key => this.values[key] instanceof ObservableValue), referenceEnhancer, `keys(${this.name})`, true));
        }
...
function defineObservableProperty(target, propName, newValue, enhancer) {
    ...
    const observable = (adm.values[propName] = new ObservableValue(newValue, enhancer, `${adm.name}.${stringifyKey(propName)}`, false));
